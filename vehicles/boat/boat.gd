...
# These are the same but mean different things, let's keep both for now
	...
	"velocity" : velo,
	"windvane": {"direction": relative_wind.angle_to(self.global_transform.basis.z),"speed": relative_wind.length()}
	}
...
func apply_drag_and_lift():
	...
	var sail_to_wind_angle = deg2rad(Globals.wind_dir) - sail.global_transform.basis.get_euler()[1] - PI/2	
	var sail_to_relative_wind_angle = relative_wind.angle_to(sail.global_transform.basis.z)
	var sail_drag = -relative_wind * sail_base_drag + relative_wind * relative_wind * sail_CD * abs(sin(sail_to_relative_wind_angle))
	...
	sail.add_force(Vector3(0,0,sail_lift).rotated(Vector3.UP, sail.global_transform.basis.get_euler()[1]), CENTER)
